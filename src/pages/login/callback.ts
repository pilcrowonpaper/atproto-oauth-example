...
export async function GET(context: APIContext): Promise<Response> {
    ...
    const authorizationServerMetadata =
        await getATProtoAuthorizationServerMetadata(issuer);
    ...
    const url = new URL(
        joinURIBaseAndPath(pdsEndpoint, "/xrpc/app.bsky.actor.getProfile")
    );
    ...
    const profileResultBytes = await readAllStreamWithLimit(
        profileResponse.body,
        1024 * 64
    );
    const profileResult = JSON.parse(
        new TextDecoder().decode(profileResultBytes)
    );
